...
def run_safe_estimation():
    ...
    if not os.path.exists(LANDMARKS_PATH):
        raise Exception("❌ landmarks.json not found. Run pose detection first.")
    ...
    # ✅ Waist width (estimated — MediaPipe has no waist landmark)
    waist_width = 0.0
    if L_SHO and L_HIP and R_SHO and R_HIP:
        left_waist = {
            "x": L_SHO["x"] + 0.40 * (L_HIP["x"] - L_SHO["x"]),
            "y": L_SHO["y"] + 0.40 * (L_HIP["y"] - L_SHO["y"]),
        }
        right_waist = {
            "x": R_SHO["x"] + 0.40 * (R_HIP["x"] - R_SHO["x"]),
            "y": R_SHO["y"] + 0.40 * (R_HIP["y"] - R_SHO["y"]),
        }
